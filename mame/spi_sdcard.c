...
void card_initialise(const char *sd1, const char *sd2)
{
	if (sd1 && strlen(sd1)) {
		...
		if (res >= 0) {
			...
			log_error("SD1: failed to open %s - %s", sd1,
				  strerror(errno));
	...
	if (sd2 && strlen(sd2)) {
		...
		if (res >= 0) {
			...
			log_error("SD2: failed to open %s - %s", sd2,
				  strerror(errno));
...
static bool card_write(int cardno, uint32_t blknext, void *data)
{
	...
	res = lseek(cards[cardno].m_harddisk, cards[cardno].m_blksize * blknext,
		    SEEK_SET);
	if (res < 0) {
		log_error("SDCARD: %d failed to seek %d", cardno,
	...
	if (res != cards[cardno].m_blksize) {
		log_error("SDCARD: %d failed to write %d", cardno, res);
...
static bool card_read(int cardno, uint32_t blknext, void *data)
{
	...
	if (res != cards[cardno].m_blksize) {
		log_error("SDCARD: %d failed to read %d", cardno, res);
		return false;
	}
...
void card_byte_in(int cardno, uint8_t m_in_latch)
{
	...
	case SD_STATE_WRITE_DATA:
		...
		if (cards[cardno].m_write_ptr ==
			...
			log_info("writing LBA %x, data %02x %02x %02x %02x",
...
uint8_t card_byte_out(int cardno)
{
	...
	log_info("SD%.1d: m_out_latch %2.2x", cardno, cards[cardno].m_out_latch);
...
void do_command(int cardno)
{
	if (((cards[cardno].m_cmd[0] & 0xc0) == 0x40) &&
		...
		log_info("SD%.1d: cmd %02d %02x %02x %02x %02x %02x",
		...
		default:
			log_error("SD%.1d: Unsupported %02x\n", cardno + 1,
				  cards[cardno].m_cmd[0] & 0x3f);
			clean_cmd = false;
			break;
		}
